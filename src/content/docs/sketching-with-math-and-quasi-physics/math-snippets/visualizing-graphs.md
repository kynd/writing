...
> 
> 
> “Graph” is an overloaded word. It can mean a plot of a function like $y=ax+b$, or a graph in graph theory, which is a network made up of nodes and edges. This page focuses on the network version of a graph.
> 
> 「グラフ（graph）」という言葉には複数の意味があります。$y=ax+b$ のような関数の“グラフ（プロット）”を指すこともあれば、グラフ理論でいう、ノードと辺からなるネットワークを指すこともあります。このページではネットワークとしてのグラフに焦点を当てます。
