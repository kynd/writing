...
To translate an object, you can simply add a vector to the position. For rotating, we could use a [2D rotation matrix](/rotation-and-trigonometry), but we will use this function based on [Rodrigues' rotation formula](https://en.wikipedia.org/wiki/Rodrigues%27_rotation_formula), which gives the rotated vector $\mathbf{v_{rot}}$ of an original vector $\mathbf{v}$ around the axis $\mathbf{k}$ by an angle $\theta$_._

位置にあるベクトルを足すだけで物体を移動させることができます。回転には、[2D回転行列](/rotation-and-trigonometry)を使用することもできますが、ここでは[Rodriguesの回転公式](https://en.wikipedia.org/wiki/Rodrigues%27_rotation_formula)に基づいたこの関数を使います。この公式は、元のベクトル$\mathbf{v}$のを軸$\mathbf{k}$を中心に角度$\theta$だけ回転したベクトル、$\mathbf{v_{rot}}$を返してくれます。

${\displaystyle {\begin{aligned} \mathbf{v_{rot}} &= \mathbf{v} \cos(\theta) + (\mathbf{k} \times \mathbf{v}) \sin(\theta) + \mathbf{k} (\mathbf{k} \cdot \mathbf{v}) (1 - \cos(\theta))\end{aligned}}}$
...
下はレイマーチングによる3Dレンダリングの例です。物体に異なる色を割り当てるには、レイからそれぞれの物体までの距離を比較して、どちらの物体にレイが当たったかを調べます。
