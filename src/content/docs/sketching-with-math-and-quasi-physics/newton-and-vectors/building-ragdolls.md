...
slug: building-ragdolls
---
Let’s use the method called Verlet Integration to create simple physics simulations. We are NOT going to implement complex features like [collision detection](/detecting-collision). Just by combining simple dots and lines, we will make a skeleton for a [dancing doll like this](https://www.instagram.com/p/CEHfUGjnJ9j/).

ベレ法という手法を使って簡単な物理シミュレーションを作ってみましょう。[衝突判定](/detecting-collision)など複雑な機能は実装しません。シンプルな点や線を部品として組み合わせて、[こんな踊る人形](https://www.instagram.com/p/CEHfUGjnJ9j/)の骨組みを作ってみます。
...
$$a_n = \dfrac{\Delta^2 x_n}{\Delta t^2} = \dfrac{(x_{n+1} - x_n) - (x_n - x_{n-1})}{\Delta t^2} = \dfrac{x_{n+1} - 2x_n + x_{n-1}}{\Delta t^2}$$

$x_{n+1} = 2 x_n - x_{n-1} + a_n \Delta t^2$

$a_n$ is the acceleration at time $n$, $x_n$ is the position, so $(x_{n+1}-x_{n}) $ is the change in position from present to slightly future, and $(x_{n}-x_{n-1})$ is the change in position from slightly past to present.

$a_n$が時刻$n$における加速度、$x_n$は位置、$(x_{n+1}-x_{n})$は現在から少し未来への位置の変化、$(x_{n}-x_{n-1})$は少し過去から現在への位置の変化です。

Or, it might be simpler to think of $(x_{n}-x_{n-1})$ approximates the current velocity, and the next position $x_{n+1}$ as the sum of the current position $x_n$ and this velocity. Please also refer to the [page on numerical differentiation](/differentiation).

$(x_{n}-x_{n-1})$ は現在の速度を近似したもの、この速度を現在の位置 $x_n$ に足したものが次の時間の位置 $x_{n+1} $と考えた方がシンプルかもしれません。[数値微分についてのページ](/differentiation)も参考にしてください。
...
Acceleration $a_n$ is not directly represented in the code, but it is implicitly incorporated. If you change the `position`, the value returned by `getVelocity` will also change.

コードでは加速度 $a_n$ は直接表現されていませんが、`position` を変更すれば`getVelocity` の返す値も変わるという形で折り込まれています。

# Sticks
# 棒

Next, we make sticks. A stick connects two points and keeps the distance between them constant. Since triangles made with three sticks are more stable and don't collapse, we can make a sort of rigid body as well.
...
See [Codifying materials (1)](/codifying-materials-1) for the texture applied to these dolls.
